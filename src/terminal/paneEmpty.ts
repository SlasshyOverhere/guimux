...
// Scans the viewport via viewportY (same field fitKeepViewport uses), so
// scrollback above the fold (e.g. cleared with `cls`) does not count.
// Every non-blank line must end in a prompt char, so command echoes,
// typed-but-unentered input, and any output all veto. Unmounted panes have
// no live buffer: live=false, empty=false, and the caller falls back to
// its dirty flag.
